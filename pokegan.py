...
try:
    import seaborn as sns  # optional; only used for plotting
except ImportError:
    sns = None
...
def show_batch(data_loader, batch_size, norm):
...
#Discriminator class to classify fake images
    ...
    for epoch in range(epochs):     #Training begins here
        ...
        if (rank == 0):
            print('Epoch: {} \tDiscriminator Loss: {:.6f} \tGenerator Loss: {:.6f} \tD(x): {:.6f} \tD(G(z)): {:.6f} / {:.6f}'.format(
                epoch + 1, avg_d_loss, avg_g_loss, avg_D_x, avg_D_G_z1, avg_D_G_z2))
...
def run_demo(callfunc, world_size):
    mp.spawn(callfunc, args=(world_size,), nprocs=world_size, join=True)
